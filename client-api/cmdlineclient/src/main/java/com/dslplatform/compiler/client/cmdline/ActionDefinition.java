...
import com.dslplatform.compiler.client.params.MonoApplicationPath;
...
public class ActionDefinition extends ActionContext implements CLCAction {
    ...
    private final static String are_diff_changes_good_raw_prompt = "Do you wish to apply this changes, quit or reload?";
    ...
    public ActionDefinition(final Api api, final Logger logger, final Output output, final Arguments arguments, final CommandLinePrompt clp, final IO io) {
    ...
    public boolean upgrade() {
        ...
        if (!updateManagedProjectResponse.authorized) {
            ...
            if (updateManagedProjectResponse.updateSuccessful) {
                output.print("Successful update!");
                return true;
            } else {
                ...
                if (updateUnsafeManagedProjectResponse.updateSuccessful) {
                    output.println("Update successful.");
                } else {
    ...
    private boolean copyToMono() {
        ...
        if (monoApplicationPath != null) {
            ...
            try {
                io.copyToDir(arguments.getRevenjPath().revenjPath, monoBin);
                io.copyToDir(arguments.getCompilationTargetPath().compilationTargetPath, monoBin);
            } catch (IOException e) {
            ...
            output.println(missing_mono_location_msg); /* todo - maybe offer to provide it in cmdline. */
            return true;
        }
    }

    private boolean writeGenerateSourcesResponseToOutputPath(final GenerateSourcesResponse generateSourcesResponse, final OutputPath outputPath) {
        if (generateSourcesResponse.generatedSuccess) {
            ...
            try {
                updateFiles(logger, generateSourcesResponse.sources, outputPath.outputPath);
                return true;
            } catch (IOException e) {
